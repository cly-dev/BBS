...
<script>
import { UploadQuestion, getQuestion, FixQuestion } from "../../api/data";
...
export default {
  ...
  async created() {
    ...
    let result = await getQuestion(`${this.questionId}`);
    this.questionId=result["data"].result[0].questionId;
    console.log(result);
    Object.defineProperty(this.ruleForm,"questionId",{
      value:result["data"].result[0].questionId,
    })
    this.$refs.editor.content = result["data"].result[0].content;
    this.ruleForm.questionname = result["data"].result[0].title;
  },
};
</script>
