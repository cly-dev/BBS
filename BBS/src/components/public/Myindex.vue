<template>
  ...
  <div class="myindex_container">
    ...
    <section class="person_container">
      ...
      <img
        :src="
          !user.selfImage ? '../../../static/images/user.png' : user.selfImage
        "
        alt="请检查网络"
      />
      ...
      <section class="person_data">
        <section class="info_person">
          ...
          <!-- 编辑个人信息 -->
          <el-button
            :style="{ right: modify ? '100px' : '' }"
            :icon="modify ? '' : 'el-icon-edit'"
            type="primary"
            @click="handleModify"
            >{{ modify ? "保存" : "编辑个人信息" }}</el-button
          >
          <el-button style="right: 20px" v-if="modify" @click="handleCanle"
            >取消</el-button
          >
          <!-- 全部信息 -->
      ...
      <section class="info_header">
        <ul>
          <router-link
            ...
            :key="index"
            :to="{ path: value.path }"
            :class="{ activer: router == value.path }"
          >
    ...
    <!-- 弹出校验框 -->
    <el-dialog title="校验" :visible.sync="visible" width="360px" center>
      <slide-verify
        ref="slideblock"
        @again="onAgain"
    ...
    <!-- 修改密码弹出框 -->
    <el-dialog title="修改密码" :visible.sync="modifyPsd" width="500px" center>
      <!-- 修改密码 -->
...
export default {
  ...
  methods: {
    ...
    async handleModifyPsd() {
      if (this.PsdFormdata.oldPsd.trim() == "") {
        ...
        if (this.PsdFormdata.newPsd === this.PsdFormdata.oldPsd) {
          ...
          if (
            CheckPsd({ value: this.PsdFormdata.newPsd }) &&
            CheckComfirmPsd(
              this.PsdFormdata.newPsd,
              this.PsdFormdata.comfirmPsd
            )
          ) {
    ...
    async onSuccess(times) {
      ...
      this.checkFlay = true;
      this.image =
        this.image == "" ? "../../../static/images/user.png" : this.image;
      //生成器
      let iterator = FixSelfInfos(this.Userinfo, this.image);
      let result = iterator.next();
      result.then((res) => {
