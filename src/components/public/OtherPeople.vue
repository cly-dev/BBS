    ...
    Promise.all([p1, p2])
      .then(res => {
        ...
        this.Userinfo.forEach((item, index) => {
          switch (index) {
            case 0:
              item.value = this.user.idolSum;
              break;
            case 1:
              item.value = this.user.fansSum;
              break;
            case 2:
              item.value = this.user.createTime;
              break;
            case 3:
              item.value = this.user.birthday;
              break;
            case 4:
              item.value = this.user.major;
              break;
            case 5:
              item.value = this.user.sex;
              break;
          }
        });
      })
            ...
            <section class="info_user">
              <section class="info_container">
                <section class="info_data">
                  ...
                  <a href="javascript:;" onClick={this.handleWatch}>
                    点击查看详细信息
                  </a>
                  <el-popconfirm
                ...
                <section class="info_detail">
                  ...
                  <span class="info_sildeUp" onClick={this.handleWatch}>
                    {" "}
                    收起
                  </span>
                </section>
                ...
                {this.nav.map(item => {
                  return (
                    <router-link
                      tag="li"
                      to={item.path}
                      class={this.router === item.path ? "activer" : ""}
                    >
                      {item.title}
                    </router-link>
                  );
