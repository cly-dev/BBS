...
export default {
  ...
  methods: {
    ...
    async handleDelete(item, index) {
      let result = "";
      if (this.index == 1) {
        result = await getQuestCollectionDelete({
          questionId: item.questionId
        });
        if (result["data"].code == "200") {
          this.message("success", "取消成功");
          this.collectionQuestion.splice(index, 1);
          this.QuestionTotal--;
          this.total--;
        } else {
        ...
        result = await CollectionArticledelete({
          articleId: item.articleId
        });
        if (result["data"].code == "200") {
          this.message("success", "取消成功");
          this.collectionArticle.splice(index, 1);
          this.ArticleTotal--;
          this.total--;
        } else {
          this.message("warning", "取消失败");
    ...
    async handleClick(e) {
      if (e.target.nodeName == "LI") {
        ...
        this.index = index;
        $(
          ".CollectionQuesition_container,.CollectionArticle_container,.CollectionAnswer_container"
        ).hide();
        if (index == 1) {
    ...
    return (
      <section>
        ...
        <main>
          ...
          <section
            class="CollectionQuesition_container"
            v-show={this.QuestionTotal != 0}
          >
            {this.collectionQuestion.map((item, index) => {
              return (
                <section class="item_contain">
                  <section class="item_data">
                    <span class="item_title">{item.title}</span>
                    <span class="item_time">发布时间:{item.createTime}</span>
                  </section>
                  <section class="item_operation">
                    <el-button
                      circle
                      icon="el-icon-search"
                      size="mini"
                      onClick={() => this.handleWatch(item)}
                    ></el-button>
                    <el-button
                      circle
                      icon="el-icon-delete"
          ...
          <section
            class="CollectionArticle_container"
            v-show={this.AnswerTotal != 0}
          >
            {this.collectionArticle.map((item, index) => {
              return (
                <section class="item_contain">
                  <section class="item_data">
                    <span class="item_title">{item.title}</span>
